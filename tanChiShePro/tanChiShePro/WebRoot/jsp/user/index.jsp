...
<head>
	...
	function init(){
		...
		userName      = index.split("&")[0].split("=")[1];
		userId        = index.split("&")[1].split("=")[1];
	...
	function snakeMove() {
		...
		if(result() == true) {
			...
			regisMsg.grade	        =	game.score;
			regisMsg.customsPass	=	$("pass").value;					
			//
			window.open("mainPage.jsp?userName="+userName
						 + "&userId=" + userId + "&isFlag=" + 1
						 + "&grade=" + game.score + "&customsPass=" + $("pass").value
						 ,"_blank"); 
			window.close();
			return;
		}
...
<div style="background-color: #21F4AD;height:550px;width:100px;">
	<label>用户名:</label><label id="userNameLabel"></label>
</div>
